! This Yasso_Myco core code is a Yasso version with implementation of mycorrhizal 
! impact on litter decomposition process. 
! This code is an extended cersion of Yasso15 code by Marko Järvenpää, which is an 
! improved version of the old Yasso07 code by M. Tuomi. 
! -Weilin Huang
...
IMPLICIT NONE
CONTAINS
SUBROUTINE mod5c_myco(theta,time,climate,init,b,d,leac,myco,NPPGPP,xt,steadystate_pred)
IMPLICIT NONE
    !********************************************* &
    ...
    ! 36-39 Myco coefficients arbuscular and ectomycorrhiza affecting the alpha terms for AWEN pools, 36&37 representing coefficient Lam and Lem for AWE pool, and 38&39 for N pool

    REAL,DIMENSION(39),INTENT(IN) :: theta ! parameters
    ...
    REAL,DIMENSION(2),INTENT(IN) :: myco ! The myco fraction for A and E
    REAL,DIMENSION(2),INTENT(IN) :: NPPGPP ! Only the second component NPPGPP(2) is used here and represents the GPP of the ecosystem. The unit is gC m-2 y-1
    ...
    REAL :: tem,temN,temH,ptem,ptemN,ptemH,size_dep,mycorr, mycorrN, myAM, myEM, myAMN, myEMN
    REAL,DIMENSION(5) :: te
    ...
    ptem = 1.0-EXP(theta(28)*climate(2)/1000.0)
    ptemN = 1.0-EXP(theta(29)*climate(2)/1000.0)
    ptemH = 1.0-EXP(theta(30)*climate(2)/1000.0)

    ! Size class dependence -- no effect if d == 0.0
    size_dep = MIN(1.0,(1.0+theta(33)*d+theta(34)*d**2.0)**(-ABS(theta(35))))


    ! Mycorrizae dependence --set mycorrhiza influence AWE pool in a similar way,
    ! N pool seperately
    mycorr = 1+(theta(36)*myco(1)+theta(37)*myco(2))*NPPGPP(2)*0.001
    mycorrN = 1+(theta(38)*myco(1)+theta(39)*myco(2))*NPPGPP(2)*0.001
    ...
    DO i = 1,3
        A(i,i) = -ABS(theta(i))*tem*size_dep*mycorr
    END DO
    A(4,4) = -ABS(theta(4))*temN*size_dep*mycorrN
	
    A(1,2) = theta(5)*ABS(A(2,2))
    A(1,3) = theta(6)*ABS(A(3,3))
    A(1,4) = theta(7)*ABS(A(4,4))
    A(1,5) = 0.0 ! no mass flows from H -> AWEN5
    ...
    ! Leaching (no leaching for humus)---adding mycorr part to seperate leaching effect
    DO i = 1,4
